...
function TaskPool({ tasks, sessions, projects, habits, occurrences, autoScheduleAssist, onCreate, onUpdate, onProgress, onCancelSession, onClose, onAutoSchedule, onCreateHabit, onScheduleHabit, onSkipHabit }: {
  ...
  useEffect(() => {
    if (!habitAnchor) return;
    // 外点关闭：点在气泡和"新建重复习惯"按钮之外时关闭；按钮自身走 onClick 的 toggle 语义
    const isOutside = (target: EventTarget | null) => {
      const el = target as HTMLElement | null;
      return !el?.closest(".habit-popover") && !el?.closest("[data-habit-button]");
    };
    const onPointerDown = (event: PointerEvent) => { if (isOutside(event.target)) setHabitAnchor(null); };
    const escape = (event: KeyboardEvent) => { if (event.key === "Escape") setHabitAnchor(null); };
    window.addEventListener("pointerdown", onPointerDown); window.addEventListener("keydown", escape);
    return () => { window.removeEventListener("pointerdown", onPointerDown); window.removeEventListener("keydown", escape); };
  }, [habitAnchor]);
  const today = toLocalDate(new Date());
...
function habitPopoverStyle(anchor: DOMRect): CSSProperties {
  const width = 250; const estimateHeight = 460;
  const vw = window.innerWidth; const vh = window.innerHeight;
  return {
    left: `${Math.max(8, Math.min(vw - width - 8, anchor.left))}px`,
    top: `${Math.max(8, Math.min(anchor.bottom + 6, vh - estimateHeight))}px`,
    width: `${width}px`,
  };
}
...
function SessionEditPopover({ session, task, anchor, onClose, onSave }: { session: ExecutionSession; task: Task; anchor: DOMRect | null; onClose: () => void; onSave: (localDate: string, startLocal: string, duration: number) => Promise<void> }) {
  ...
  useEffect(() => {
    // 外点关闭：点在气泡和编辑按钮之外时关闭；按钮自身走 onClick 的 toggle 语义
    const isOutside = (target: EventTarget | null) => {
      const el = target as HTMLElement | null;
      return !el?.closest(".session-edit-popover") && !el?.closest("[data-session-edit-button]");
    };
    const onPointerDown = (event: PointerEvent) => { if (isOutside(event.target)) onClose(); };
    const escape = (event: KeyboardEvent) => { if (event.key === "Escape") onClose(); };
    window.addEventListener("pointerdown", onPointerDown); window.addEventListener("keydown", escape);
    return () => { window.removeEventListener("pointerdown", onPointerDown); window.removeEventListener("keydown", escape); };
  }, [onClose]);
  const width = 280; const estimateHeight = 400;
  const vw = typeof window !== "undefined" ? window.innerWidth : 1024;
  const vh = typeof window !== "undefined" ? window.innerHeight : 768;
  const pos = anchor
    ? { left: Math.max(8, Math.min(vw - width - 8, anchor.right - width)), top: anchor.bottom + estimateHeight + 12 > vh ? Math.max(8, anchor.top - estimateHeight - 6) : anchor.bottom + 6 }
    : { left: Math.max(8, Math.round((vw - width) / 2)), top: Math.max(12, Math.round((vh - estimateHeight) / 2)) };
  ...
  return createPortal(<section className="session-edit-popover" role="dialog" aria-modal="false" aria-label={`编辑时间：${task.title}`} style={{ top: `${pos.top}px`, left: `${pos.left}px`, width: `${width}px` }}>
