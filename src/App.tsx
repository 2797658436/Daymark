...
function DayCalendarGrid({ date, workspace, preferences, now, focusSessionId, onSchedule, onCreateTaskAt, onMove, onPlace, onProgress, onSkipReview, onContinue, onCreateBlock, onUpdateBlock, onDeleteBlock, onEditSession, onDragGhost }: {
  ...
  onPlace: (intent: CalendarDropIntent) => Promise<void>;
  onProgress: (task: Task, value: number) => Promise<void>; onSkipReview: (session: ExecutionSession) => Promise<void>; onContinue: (session: ExecutionSession) => void;
  onCreateBlock: (block: TimeBlock) => Promise<void>; onUpdateBlock: (previous: TimeBlock, next: TimeBlock) => Promise<void>; onDeleteBlock: (id: string) => Promise<void>; onEditSession: (session: ExecutionSession) => void; onDragGhost?: (ghost: { leftPx: number; topPx: number; widthPx: number; heightPx: number; mode: CalendarDropMode; targetSessionId: string; title: string } | null) => void;
}) {
