import {
  ArchiveRestore, ArrowDown, ArrowUp, Bell, CalendarDays, Check, ChevronDown, ChevronLeft, ChevronRight, ChevronUp, CircleAlert,
...
function AppShell({ settings: injectedSettings, native: injectedNative }: AppProps) {
  ...
  const [narrowWindow, setNarrowWindow] = useState(() => window.innerWidth < 1200);
  useEffect(() => { const resize = () => setNarrowWindow(window.innerWidth < 1200); window.addEventListener("resize", resize); return () => window.removeEventListener("resize", resize); }, []);
  ...
  const updatePreferences = useCallback((patch: Partial<AppSettings>) => {
    ...
    if (patch.lastPage) { setPage(patch.lastPage); setPoolOverlayOpen(false); }
  }, [persist]);
  ...
  const poolUsesOverlay = narrowWindow && !(page === "calendar" && preferences.calendarView === "day");
  const showTaskPool = hasTaskPool && (poolUsesOverlay ? poolOverlayOpen : taskPoolOpen);
  ...
  return (
    <div className={`app-shell ${showTaskPool ? "with-task-pool" : "without-task-pool"} ${dayViewPool && showTaskPool ? "day-view-pool" : ""}`} style={preferences.taskPoolWidth ? ({ "--pool-width": `${preferences.taskPoolWidth}px` } as CSSProperties) : undefined}>
      ...
      <main id="main-content" className={`workspace-main ${page === "calendar" ? "calendar-workspace" : ""}`}>
        ...
        {page === "projects" && <ProjectsPage workspace={workspace} onCreateTask={async (task) => { await commit(() => native.createTask(task)); }} onArrange={(task) => { setSchedulingTaskId(task.id); setSchedulingOpen(true); }} onCreate={async (project, tasks) => { await commit(() => native.createProjectWithTasks(project, tasks)); }}
        ...
        {hasTaskPool && !showTaskPool && <button className="task-pool-toggle" aria-label="打开任务池" onClick={() => poolUsesOverlay ? setPoolOverlayOpen(true) : setTaskPoolOpen(true)}><Inbox size={18} /><span>任务池</span></button>}
      </main>
      ...
      {showTaskPool && (
        ...
        <button className="task-pool-scrim" aria-label="关闭任务池浮层" onClick={() => poolUsesOverlay ? setPoolOverlayOpen(false) : setTaskPoolOpen(false)} />
        <TaskPool tasks={activeTasks} sessions={workspace.executionSessions} projects={workspace.projects} habits={workspace.recurringHabits} occurrences={workspace.habitOccurrences} autoScheduleAssist={preferences.autoScheduleAssist}
          onCreate={createTask} onUpdate={async (task) => { await commit(() => native.updateTask(task)); }} onProgress={updateProgress} onCancelSession={cancelSession} onClose={() => poolUsesOverlay ? setPoolOverlayOpen(false) : setTaskPoolOpen(false)} onAutoSchedule={() => setSchedulingOpen(true)} onCreateHabit={createHabit} onScheduleHabit={scheduleHabit} onSkipHabit={skipHabit} />
        </>
      )}
      ...
      {schedulingOpen && <AutoScheduleDialog initialTaskId={schedulingTaskId} workspace={workspace} preferences={preferences} onClose={() => { setSchedulingOpen(false); setSchedulingTaskId(null); }} onApply={async (sessions, occurrences) => { await commit(() => native.applyScheduleDraft(sessions, occurrences)); pushUndo({ label: "撤销本次自动排程", run: () => native.deleteExecutionSessions(sessions.map((session) => session.id)) }); setSchedulingOpen(false); setSchedulingTaskId(null); }} />}
    </div>
  );
}
...
function TodayPage({ workspace, preferences, running, onStart, onFinish, onProgress, onGoCalendar, onDelay, onSkip, onSkipReview }: {
  ...
  onSkipReview: (session: ExecutionSession) => Promise<void>;
  onDelay: (session: ExecutionSession) => Promise<void>; onSkip: (session: ExecutionSession) => Promise<void>;
}) {
  const now = useCurrentTime();
  const today = toLocalDate(now);
  const sessions = workspace.executionSessions.filter((item) => item.localDate <= today && item.endLocalDate >= today && item.status === "scheduled").sort(compareSessions);
  const missed = workspace.executionSessions.filter((item) => item.localDate <= today && (item.endLocalDate ?? item.localDate) === today && (item.status === "missed" || isPendingReview(item, workspace.tasks.find((task) => task.id === item.taskId), workspace.executionRecords, now))).sort(compareSessions);
  const nowMinutes = new Date().getHours() * 60 + new Date().getMinutes();
  const current = sessions.find((session) => sessionContains(session, now))
    ?? sessions.find((session) => localSessionStart(session) > now.getTime());
  ...
  return <section className="page-stack" aria-labelledby="today-title">
    <PageHeader eyebrow={formatLongDate(today)} title="今天从下一步开始" actions={<div className="summary-pills"><span>{due.length} 项临期</span><span>进度变化 {progressed} 项 · {delta >= 0 ? "+" : ""}{delta} 百分点</span></div>} />
    ...
    <div className="today-grid">
      ...
      <section className="surface-card"><div className="section-heading"><h2>待回顾与临期</h2><span>{missed.length} 个待回顾／未推进时段</span></div>
        {(missed.length > 0 || due.length > 0) ? <div className="attention-list">{missed.map((session) => <div key={session.id}><div><strong>{taskTitle(workspace, session.taskId)}</strong><span>{session.status === "missed" ? "本次未推进" : "待回顾"} · {session.startLocal}–{session.endLocal}</span></div><div><Button onClick={() => onGoCalendar(session)}>更新进度</Button><Button onClick={() => onGoCalendar(session)}>继续安排</Button>{session.status === "scheduled" && <Button onClick={() => void onSkipReview(session)}>本次未推进</Button>}</div></div>)}{due.sort(compareDeadlines).map((task) => <div key={task.id}><div><strong>{task.title}</strong><span>{deadlineLabel(today, task.deadlineLocal!)}</span></div><ProgressControl task={task} onCommit={onProgress} /></div>)}</div>
...
function TaskPool({ tasks, sessions, projects, habits, occurrences, autoScheduleAssist, onCreate, onUpdate, onProgress, onCancelSession, onClose, onAutoSchedule, onCreateHabit, onScheduleHabit, onSkipHabit }: {
  ...
  const assistCount = tasks.filter((task) => !sessions.some((session) => session.taskId === task.id && hasFutureSchedule(session))).length
    ...
    <div className="task-list project-groups">{groups.map(({ key, project, tasks: groupTasks }) => {
      ...
      return <section key={key} className="project-group">
        ...
        <div className={isExpanded ? "project-group-body expanded" : "project-group-body"}>
          {!isExpanded && !fullyCollapsed && current && groupTasks.length > 1 && <p className="empty-inline">显示下一项 · 展开查看全部 {groupTasks.length} 项</p>}{shown.map((task) => <TaskPoolCard key={task.id} task={task} sessions={sessions} onProgress={onProgress} onEdit={toggleEditor} />)}
...
function CalendarPage({ workspace, preferences, focusSessionId, onPreferences, onSchedule, onCreateTaskAt, onMove, onPlace, onProgress, onSkipReview, onCreateBlock, onUpdateBlock, onDeleteBlock }: {
  ...
  useLayoutEffect(() => { if (preferences.calendarView === "month") { const viewport = calendarRef.current?.querySelector<HTMLElement>(".calendar-viewport"); if (viewport) viewport.scrollTop = 0; } }, [preferences.calendarView]);
...
function ContinuousDayView({ anchor, workspace, preferences, now, focusSessionId, onAnchorChange, onSchedule, onCreateTaskAt, onMove, onPlace, onProgress, onSkipReview, onContinue, onCreateBlock, onUpdateBlock, onDeleteBlock, onEditSession }: {
  ...
  useEffect(() => { const clearGhost = () => setDayGhost(null); window.addEventListener("dragend", clearGhost); window.addEventListener("drop", clearGhost); return () => { window.removeEventListener("dragend", clearGhost); window.removeEventListener("drop", clearGhost); }; }, []);
  useEffect(() => {
  ...
  return <div ref={axisRef} className="continuous-day-axis" role="region" aria-label="连续日时间轴" onScroll={onScroll}>
    {[addDays(anchor, -1), anchor, addDays(anchor, 1)].map((date) => <section key={date} className="continuous-day-panel" data-calendar-date={date} aria-label={formatLongDate(date)}>
      ...
      <DayCalendarGrid date={date} workspace={workspace} preferences={preferences} now={now} focusSessionId={focusSessionId} onSchedule={onSchedule} onCreateTaskAt={onCreateTaskAt} onMove={onMove} onPlace={onPlace} onProgress={onProgress} onSkipReview={onSkipReview} onContinue={onContinue} onCreateBlock={onCreateBlock} onUpdateBlock={onUpdateBlock} onDeleteBlock={onDeleteBlock} onEditSession={onEditSession} onDragGhost={(g) => setDayGhost(g)} />
    </section>)}
    {dayGhost && dayGhost.mode === "place" && <div className="calendar-drag-ghost" role="status" aria-label="拖拽排程预览" style={{ left: `${dayGhost.leftPx}px`, top: `${dayGhost.topPx}px`, width: `${dayGhost.widthPx}px`, height: `${dayGhost.heightPx}px` }}><span>{dayGhost.targetSessionId ? "悬停 0.5 秒以同时安排" : "松开放置"}</span></div>}
  </div>;
}
...
function DayCalendarGrid({ date, workspace, preferences, now, focusSessionId, onSchedule, onCreateTaskAt, onMove, onPlace, onProgress, onSkipReview, onContinue, onCreateBlock, onUpdateBlock, onDeleteBlock, onEditSession, onDragGhost }: {
...
function CalendarDay({ date, workspace, preferences, now, timeline, expandedGapKeys, dragExpandedGapKey, focusSessionId, onToggleGap, onGapDragEnter, onGapDragLeave, onClearDragExpansion, onRetainDragExpansion, hideHeader = false, selected = false, weekGridTabIndex, weekGridMinute, onWeekGridFocus, onWeekGridNavigate, onSchedule, onCreateTaskAt, onMove, onPlace, onProgress, onSkipReview, onContinue, onCreateBlock, onUpdateBlock, onDeleteBlock, onEditSession, onDragGhost }: {
  ...
  const [blankBusy, setBlankBusy] = useState(false); const [blankError, setBlankError] = useState("");
  ...
  const previewFromDrag = (event: DragEvent<HTMLDivElement>) => {
    ...
    if (target && targetCard) {
      ...
      if (ratio >= .25 && ratio <= .75 && overlapTarget.current !== target.id) {
        ...
        overlapTimer.current = window.setTimeout(() => { setDragPreview((current) => current && current.targetSessionId === target.id ? { ...current, mode: "overlap", startMinute: timeMinutes(target.startLocal) } : current); onDragGhost?.(null); overlapTimer.current = null; }, 500);
  ...
  return <div className={dayClasses} data-day-date={date}>{!hideHeader && <header>{heading}</header>}
    <div className={timeline ? "day-track folded-timeline" : "day-track"} role={onWeekGridNavigate ? "gridcell" : undefined} tabIndex={weekGridTabIndex} aria-label={onWeekGridNavigate ? `${date} ${minutesTime(weekGridMinute ?? 540)} 空白时间` : undefined} aria-selected={onWeekGridNavigate ? selected : undefined} onFocus={(event) => { if (event.target === event.currentTarget) onWeekGridFocus?.(); }} onKeyDown={(event) => { if (event.target !== event.currentTarget || !onWeekGridNavigate) return; if (event.key === "Enter") { event.preventDefault(); const start = weekGridMinute ?? 540; setBlankRange({ start, end: Math.min(1440, start + 30) }); setBlankAction("menu"); return; } onWeekGridNavigate(event); }} style={timeline ? { height: `${timeline.totalHeight}px` } : undefined}
      ...
      {blankRange && blankAction && <FloatingPanel label="空白时段操作" kind="action" anchor={blankAnchor} onClose={() => { setBlankAction(null); setBlankRange(null); }} closeOnOutsideClick><section ref={blankBubbleRef} aria-busy={blankBusy} className="blank-action-bubble" onPointerDown={(event) => event.stopPropagation()}>
        ...
        {blankAction === "pool" && <div className="blank-task-picker">{workspace.tasks.filter((task) => task.status === "active" && task.kind !== "habit").map((task) => <button disabled={blankBusy} key={task.id} onClick={() => { if (blankBusy) return; setBlankBusy(true); setBlankError(""); void onSchedule(task, date, minutesTime(blankRange.start)).then(() => { setBlankAction(null); setBlankRange(null); }).catch((reason) => setBlankError(readError(reason))).finally(() => setBlankBusy(false)); }}><span className="blank-task-icon" aria-hidden="true"><Inbox size={14} /></span><span className="blank-task-text"><strong>{task.title}</strong><small>{task.sessionMinutes ?? task.estimatedMinutes ?? 60} 分钟</small></span></button>)}{!workspace.tasks.some((task) => task.status === "active" && task.kind !== "habit") && <span className="empty-inline">任务池暂无可安排任务</span>}</div>}
        {blankError && <p role="alert">{blankError}</p>}
      </section></FloatingPanel>}
      {dragPreview && (() => {
        // 虚线框跟鼠标 y 轴（snap 到 15 分钟格线 = drop 落点指示）；占位符（source 半透明）静止在原位置
        ...
        const previewStyle = timeline ? timeline.positionForRange(start, end) : positionStyle(start, end);
      ...
      {expandedConcurrency && (() => {
        ...
        return <section className="concurrent-expanded-list" aria-label="同时安排的全部任务" style={timeline ? { top: `${top}px` } : { top: `${top}%` }}>{groupSessions.map((session) => <button key={session.id} onClick={() => onEditSession(session, null)}><strong>{taskTitle(workspace, session.taskId)}</strong><time>{session.startLocal}–{session.endLocal}</time></button>)}</section>;
      })()}
 ...
 * 默认时段编辑器（P4-03）。
 *
 * 偏好写入仍走 `onChange`，不引入新的保存协议；这里只统一草稿、提交互斥与失败保留。
 * 时段为空数组是有效的用户选择，因此「删除」不经过本编辑器。
 */
function DefaultTimeSlotEditor({ slot, onCancel, onSave, onPhase }: {
  slot: DefaultTimeSlot; onCancel: () => void;
  onSave: (slot: DefaultTimeSlot) => Promise<void>; onPhase?: (busy: boolean) => void;
}) {
  const session = useEditorSession<DefaultTimeSlot>({
    key: slot.id,
    initial: slot,
    canSave: (draft) => Boolean(draft.label.trim() && draft.weekdays.length && draft.start !== draft.end),
    onSave: (draft) => onSave(draft),
    onClose: (reason) => { if (reason !== "saved") onCancel(); },
    onPhaseChange: (phase) => onPhase?.(phase === "submitting"),
  });
  const { draft, patchDraft, busy } = session;
  return <form onSubmit={(event) => { event.preventDefault(); void session.save(); }}>
    <h2>默认时段</h2>
    <label>名称<input required value={draft.label} onChange={(event) => patchDraft({ label: event.target.value })} /></label>
    <label>开始<input required type="time" value={draft.start} onChange={(event) => patchDraft({ start: event.target.value })} /></label>
    <label>结束<input required type="time" value={draft.end} onChange={(event) => patchDraft({ end: event.target.value })} /></label>
    <fieldset><legend>适用星期</legend><div className="weekday-checks">{[1, 2, 3, 4, 5, 6, 0].map((day) => <label key={day}><input type="checkbox" checked={draft.weekdays.includes(day)} onChange={(event) => patchDraft({ weekdays: event.target.checked ? [...draft.weekdays, day] : draft.weekdays.filter((value) => value !== day) })} />{["日", "一", "二", "三", "四", "五", "六"][day]}</label>)}</div></fieldset>
    <div className="form-actions"><Button disabled={busy} onClick={session.cancel}>取消</Button><Button type="submit" variant="primary" disabled={busy || !session.canSave}>{busy ? "正在保存…" : "保存时段"}</Button></div>
  </form>;
}
...
function AutoScheduleDialog({ workspace, preferences, onClose, onApply, initialTaskId }: { initialTaskId?: string | null; workspace: WorkspaceSnapshot; preferences: AppSettings; onClose: () => void; onApply: (sessions: ExecutionSession[], occurrences: HabitOccurrence[]) => Promise<void> }) {
  ...
  const [selected, setSelected] = useState(() => new Set(recommended.allocations.filter((allocation) => !initialTaskId || allocation.taskId === initialTaskId).map((allocation) => allocation.key)));
...
function ReviewPage({ workspace, onGoToday }: { workspace: WorkspaceSnapshot; onGoToday: () => void }) {
  ...
  const hasFacts = review.progressedTaskIds.length > 0 || totalMinutes > 0 || totalDelta !== 0 || review.days.some((day) => day.missedCount + day.skippedCount > 0);
  return <section className="page-stack" aria-labelledby="review-title"><PageHeader eyebrow={`${formatShortDate(review.startDate)} — ${formatShortDate(review.endDate)}`} title="最近 7 天回顾" actions={<div className="summary-pills"><span>实际投入 {totalMinutes} 分钟</span><span>净进度 {totalDelta >= 0 ? "+" : ""}{totalDelta} 百分点</span></div>} />{hasFacts ? <><div className="review-outcomes"><section className="surface-card"><h2>完成与推进</h2><p>{review.completedTaskIds.length} 项曾完成 · {review.progressedTaskIds.length} 项有进度变化</p><div className="review-task-list">{review.progressedTaskIds.map((id) => <span key={id}>{taskTitle(workspace, id)}</span>)}</div></section><section className="surface-card"><h2>待续</h2><p>{review.carryoverTaskIds.length} 项仍可继续安排</p><div className="review-task-list">{review.carryoverTaskIds.map((id) => <span key={id}>{taskTitle(workspace, id)}</span>)}</div></section></div><section className="surface-card"><div className="section-heading"><h2>每天的事实</h2><span>不评分，不计算连续天数</span></div><div className="review-days">{review.days.map((day) => <article key={day.date}><time>{weekday(day.date)}<small>{formatShortDate(day.date)}</small></time><div className="review-bar"><span style={{ width: `${Math.min(100, Math.max(0, day.actualMinutes / Math.max(1, ...review.days.map((item) => item.actualMinutes)) * 100))}%` }} /></div><p>进度 {day.progressDelta >= 0 ? "+" : ""}{day.progressDelta} 百分点 · 投入 {day.actualMinutes} 分钟 · 未执行／跳过 {day.missedCount + day.skippedCount}</p></article>)}</div></section></> : <EmptyState icon={<ChartNoAxesColumnIncreasing />} title="有实际推进后，这里会出现回顾" text="没有记录时不绘制全零图表，也不会把未记录解释为失败。" action={<Button variant="primary" onClick={onGoToday}>前往今日</Button>} />}</section>;
}
...
function DataPage({ native }: { native: NativeApi }) {
  ...
  const [currentCounts, setCurrentCounts] = useState<{ projects: number; tasks: number } | null>(null);
...
function BackupRow({ backup, native, disabled, onPreview }: { backup: BackupInfo; native: NativeApi; disabled: boolean; onPreview: (preview: BackupPreview) => void }) {
  const [preview, setPreview] = useState<BackupPreview | null>(null); const [error, setError] = useState("");
  useEffect(() => { let alive = true; void native.inspectBackup(backup.path).then((value) => { if (alive) setPreview(value); }).catch((reason) => { if (alive) setError(readError(reason)); }); return () => { alive = false; }; }, [backup, native]);
  return <div><Database /><div><strong>{backup.kind === "daily" ? "每日备份" : backup.kind === "manual" ? "手动备份" : "恢复前备份"}</strong>{preview ? <p>{new Date(preview.modifiedAt).toLocaleString()} · {(preview.sizeBytes / 1024).toFixed(1)} KB · 检查通过</p> : <p>{error || "正在检查备份…"}</p>}<details><summary>文件位置</summary><span>{backup.path}</span></details><Button disabled={disabled || !preview} onClick={() => preview && onPreview(preview)}>检查并预览</Button></div></div>;
}
...
function applyDisplayPreferences(settings: AppSettings, systemDark: boolean, systemReduced: boolean) { const root = document.documentElement; root.dataset.theme = settings.appearance === "system" ? (systemDark ? "dark" : "light") : settings.appearance; root.dataset.motion = settings.motion === "system" ? (systemReduced ? "reduce" : "full") : settings.motion; root.dataset.scale = settings.scale >= 150 ? "large" : "normal"; root.style.fontSize = `${settings.scale}%`; }
function readError(error: unknown) { return error instanceof Error ? error.message : String(error); }

function isPendingReview(session: ExecutionSession, task: Task | undefined, records: ExecutionRecord[], now: Date) {
  return session.status === "scheduled" && sessionEnded(session, now) && !!task && task.status !== "completed" && task.progress < 100 && !records.some((record) => record.sessionId === session.id);
}

function formatDuration(minutes: number) { return `${Math.floor(minutes / 60)} 小时 ${minutes % 60} 分钟`; }

function SchedulePreview({ allocations }: { allocations: ScheduleAllocation[] }) {
  const dates = [...new Set(allocations.map((item) => item.localDate))].sort();
  return <div className="schedule-preview">{dates.map((date) => { const items = allocations.filter((item) => item.localDate === date).sort((a, b) => a.startLocal.localeCompare(b.startLocal)); return <section key={date}><h3>{formatShortDate(date)} · {items.length} 个时段 · {formatDuration(items.reduce((sum, item) => sum + item.minutes, 0))}</h3>{items.map((item) => <article key={item.key}><time>{item.startLocal}–{item.endLocal}</time><strong>{item.title}</strong><span>{item.minutes < item.targetMinutes ? `先安排 ${item.minutes}/${item.targetMinutes} 分钟` : formatDuration(item.minutes)}</span></article>)}</section>; })}{!dates.length && <p>当前选择没有可应用的空档。</p>}</div>;
}

function hasFutureSchedule(session: ExecutionSession) { return session.status === "scheduled" && localSessionStart(session) > Date.now(); }
