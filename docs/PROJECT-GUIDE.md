...
- 时间块拖动整块时纵向自由跟手、横向按整列吸附；跨列落点先隐藏源列那一份、再 portal 进目标列的轨道渲染，直到写入回来 —— 松手不会弹回源列再跳一次。
...
| [`src/lib/calendarDrag.ts`](../src/lib/calendarDrag.ts) | 拖拽负载的影子副本与类型判定：绕开 `dragenter`／`dragover` 期间 `dataTransfer.getData()` 的保护模式空串，供整块拖拽预览、插入线与落点提示读取。 |
...
| Vitest | 设置规范化／排序（含任务池宽度、零个／多个默认时段、日视图模式、吸附关闭、日历三视图锚点／连续比例／实际记录开关）、课程解析、自动排程、重复规则、7 天回顾、B 站解析、今日壳、任务池、挽救动作、精确时间编辑、拖拽负载影子副本与类型判定，以及日历三视图、连续日边界、当前状态、实际叠加、滚轮缩放、卡片信息分层、折叠布局映射、任务截止紧迫度、月摘要、全天溢出、周／月键盘导航与目标显露、跨列落点在目标列渲染；M7 的项目截止编辑、里程碑 CRUD 与达成态、项目截止／里程碑日历标记、到期结果展示、累计目标续排、冻结历史不可改写与项目／里程碑排程约束。 |
| axe-core | jsdom 语义检查，以及 Playwright 中浅／深色真实浏览器自动无障碍检查。 |
| Playwright | 快速创建重载无重复、拖拽安排与 Ctrl+Z、时段移动／取消、课程导入、阶段 2 排程／时间块／习惯持久化、执行结束重载、浅／深色 axe、200% 缩放、窄窗布局与日视图任务池调宽持久化、**真实指针拖拽下拖拽源保持可命中**、待回顾气泡只在明确手势后打开，以及阶段 3 视图状态、连续日边界、M2 叠加、M3 缩放事实、M4 折叠边界、M5 插入原子提交与空白框选、M6 月摘要／详情浮层／全天吸顶／键盘导航。 |
