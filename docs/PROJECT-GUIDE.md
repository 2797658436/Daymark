...
- 时间块拖动整块时纵向自由跟手、横向按整列吸附并半透明；跨列落点先隐藏源列那一份、再 portal 进目标列的轨道渲染，直到写入回来 —— 松手不会弹回源列再跳一次。已排卡片走同一套「落点先画出来」的规则（`pendingPlacement`）：目标列按落点渲染、源列同时让位。落点虚线框高度按真实时长算，但**不得矮于卡片**（最小 32px），换格时用 150ms 过渡扫过格线而不是瞬移。
