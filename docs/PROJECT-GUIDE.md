...
- 日／周拖拽持续显示精确起止时间与落点；工具栏磁铁按钮显示当前吸附状态，`Alt` 临时反转吸附。已有卡片边缘显示插入线和“插入并后移”预览，中央悬停约 500ms 切换为“同时安排”；拖离只清除内存预览，松手后才通过 `applyExecutionSessionChanges` 单事务提交，并以一个撤销项整体恢复。并行 2–3 项等宽并排，4 项以上保留前 2 项和“另外 N 项”入口。空白悬停显示 `＋` 与时间，单击或纵向框选打开预填范围的三入口气泡。拖动中的源卡片只变淡，**不得被设为不可命中**（`pointer-events: none` 会让浏览器在 `dragstart` 后立刻取消整次拖拽）；`dragenter`／`dragover` 期间 `dataTransfer.getData()` 按规范返回空串，正在拖的内容由 `lib/calendarDrag.ts` 的影子副本提供，`drop` 阶段才读 `dataTransfer` 真值。
- 时间块拖动整块时纵向与横向都**自由跟手**（贴着指针走）、并且半透明；落点虚线框另外按整列吸附，两者分离才看得出最终落在哪一列哪一格。松手后卡片瞬时收到虚线框里（吸附后的时间 + 吸附后的列，不加过渡），等 workspace 追上来时目标列渲染的真身与之完全重合 —— 既没有弹回源列再跳一次的过程，也不必把元素搬进另一列的 DOM。已排卡片走同一套「落点先画出来」的规则（`pendingPlacement`）：目标列按落点渲染、源列同时让位。落点虚线框高度按真实时长算，但**不得矮于卡片**（最小 32px），换格时用 150ms 过渡扫过格线而不是瞬移。
...
- 已结束、未完成且尚无已结束执行记录的时段显示“待回顾”，可更新进度、确认后续安排或由用户明确记录“本次未推进”；时间经过本身不再自动写入 `missed`／未执行事实。三个动作收在 `FloatingPanel`（`kind="action"`）里，**只在明确手势后打开**：点卡片本体，或聚焦卡片上的“待回顾”入口按钮后回车；Esc／点外部／动作完成关闭。卡片上的 `Enter` 仍是“编辑时间”，不因待回顾而改变。
...
本轮依据 [GUI 评估](reports/gui-evaluation-2026-09-06.md) 调整呈现与交互，实施范围与验证见 [实施记录](reports/gui-optimization-2026-09-08.md)。SQLite 仍为 schema v7。

- `src/components/ui/floating-panel.tsx` 统一项目、项目任务、里程碑、任务编辑、习惯、时间块、日历空白操作与默认时段的浮层边界、内容测量和焦点恢复。多字段任务编辑改为明确保存／取消；表单外点击保留当前草稿，不触发背景操作。
- 今日与日历复用待回顾判断；月历和回顾使用包含下调的净进度变化，单位为百分点，曾完成与当前完成度分别表达。任务池“下次”及筛选只计未来尚未开始的有效安排。
- 窄窗任务池的临时浮层开关与侧栏开关分开，跨页关闭临时浮层；设置按缩放后的可用空间重排。默认时段呈现摘要行，编辑后确认保存。
- 项目内可直接添加任务、搜索与筛选未完成项、定位最近推进位置、进入单项排程。项目任务的进度控件按需展开；导入文本、链接与项目标题按模式保留独立草稿。
- 排程支持搜索／项目过滤与批量选择，草案按日期分组、时间排序并显示每天投入。原有应用与撤销事务保持不变。
- 备份列表逐项只读检查，显示日期、大小与检查结果；`BackupPreview.includesPreferences` 表示设置包含情况。恢复确认同时显示当前与目标数量；替换、恢复前备份和回滚协议未改变。
...
| [`src/lib/calendarDrag.ts`](../src/lib/calendarDrag.ts) | 拖拽负载的影子副本与类型判定：绕开 `dragenter`／`dragover` 期间 `dataTransfer.getData()` 的保护模式空串，供整块拖拽预览、插入线与落点提示读取；抓取偏移统一在这里取值并保证有限。 |
| [`src/lib/timelineDrag.ts`](../src/lib/timelineDrag.ts) | 拖拽／缩放共用的时间换算：吸附粒度与 `Alt` 反转、格线吸附与钳制、落点指示框最小高度。此前同一套规则在四处各写一份。 |
| [`src/lib/pendingPlacement.ts`](../src/lib/pendingPlacement.ts) | 「松手之后、写入回来之前」那次放置的共享状态：目标列按落点先画一份，源列把同一份藏起来，避免跨列松手时的错位动画；`useSyncExternalStore` 让并列的列同时看到。 |
| [`src/components/ui/drag-time-preview.tsx`](../src/components/ui/drag-time-preview.tsx) | 拖动／改时长时贴着卡片的浮动时间读数（越界翻转、贴边、跟随锚点），时间块与卡片共用一份。 |
...
| Vitest | 设置规范化／排序（含任务池宽度、零个／多个默认时段、日视图模式、吸附关闭、日历三视图锚点／连续比例／实际记录开关）、课程解析、自动排程、重复规则、7 天回顾、B 站解析、今日壳、任务池、挽救动作、精确时间编辑、拖拽负载影子副本与类型判定、时间换算与吸附、落点覆盖的收敛判定，以及日历三视图、连续日边界、当前状态、实际叠加、滚轮缩放、卡片信息分层、折叠布局映射、任务截止紧迫度、月摘要、全天溢出、周／月键盘导航与目标显露、跨列落点在目标列渲染；M7 的项目截止编辑、里程碑 CRUD 与达成态、项目截止／里程碑日历标记、到期结果展示、累计目标续排、冻结历史不可改写与项目／里程碑排程约束。 |
| axe-core | jsdom 语义检查，以及 Playwright 中浅／深色真实浏览器自动无障碍检查。 |
| Playwright | 快速创建重载无重复、拖拽安排与 Ctrl+Z、时段移动／取消、课程导入、阶段 2 排程／时间块／习惯持久化、执行结束重载、浅／深色 axe、200% 缩放、窄窗布局与日视图任务池调宽持久化、**真实指针拖拽下拖拽源保持可命中**、**落点虚线框不矮于卡片且换格有过渡**、待回顾气泡只在明确手势后打开，以及阶段 3 视图状态、连续日边界、M2 叠加、M3 缩放事实、M4 折叠边界、M5 插入原子提交与空白框选、M6 月摘要／详情浮层／全天吸顶／键盘导航。 |
