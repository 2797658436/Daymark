...
### 3.5 补充（2026-09-20）：两个因素都已确认并修复

就地插桩（`console.log` 记录 `zoomWithWheel` 的锚点参数与补偿分支，跑 45 次）后确认，这是**两个独立成因**，不是一个：

1. **挂载后的自动定位会覆盖先发生的滚动**。`App.tsx` 的首次"定位到当前时间"走双 `requestAnimationFrame`，其守卫只防自身重复，不感知外部对 `scrollTop` 的写入，因此会晚于用例的程序化写入落地并把位置拽走。**处置**：自动定位在落地前若收到该滚动容器上的任何 `scroll` 事件即自行作废（用户与程序化滚动都优先）。此修复使 M4 由间歇失败变为 15/15 通过。
2. **`onWheel` 是 React 合成事件，在根节点被注册为 passive**，处理器里的 `preventDefault()` 因此无效，浏览器仍会执行自己的默认滚动，并且**可能先于处理器改变 `scrollTop`**。处理器随后用 `getBoundingClientRect` 取到的锚点比例就偏了：实测 `anchorRatio` 由 0.5 变为 0.4135（对应 09:55），补偿结果由 407 变为 299 —— 位移恰好 108px，正与本报告第 3 节反解出的 09:55 吻合。**处置**：改用 `addEventListener("wheel", handler, { passive: false })` 的原生监听，让 `preventDefault()` 真正生效。

**这与本报告第 3.2 节的结论不矛盾**：缩放数学本身正确，坏掉的是"测量锚点时的滚动位置"。两个成因叠加后才表现为 108px 的稳定位移。

修复后实测：M3 `--repeat-each=30` → **30 通过 / 0 失败**；M4 `--repeat-each=15` → **15 通过 / 0 失败**（修复前 M3 基线为 15 次中 2 次失败、20 次中 1 次失败）。用例侧同时补了 `waitForScrollSettled`：缩放前先确认滚动位置连续多帧不变，把"测量前位置必须稳定"这一前提显式化。**未放宽任何断言。**
